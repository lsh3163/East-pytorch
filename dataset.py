...
def move_points(vertices, index1, index2, r, coef):
        ...
        Input : 
            ...
            r : [r1, r2, r3, r4] in paper -> reference length for shrinking method
            coef : shrink ratio in paper
        Output:
            vertices : where one edge has been shrinked
        '''
        index1 = index1 % 4
        index2 = index2 % 4
        x1_index = index1 * 2 + 0
...
def get_score_geo_quad(img, vertices, labels, scale, length):
        ...
        '''
        score_map   = np.zeros((int(img.height * scale), int(img.width * scale), 1), np.float32)
        geo_map     = np.zeros((int(img.height * scale), int(img.width * scale), 8), np.float32)
        ignored_map = np.zeros((int(img.height * scale), int(img.width * scale), 1), np.float32)
...
class custom_dataset(data.Dataset):
        def __init__(self, img_path, gt_path, scale=0.25, length=512, mode="QUAD"):
